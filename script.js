...
// Section H2s fade/slide in every time they scroll into view — and
// back out (removing .in-view) once they leave, so scrolling back up
// or down past one replays the animation instead of it firing only
// once (see the ".h2-reveal" comment in style.css for why the hidden
// starting state is applied here in JS rather than living directly on
// the base h2 selector — it keeps headings visible by default if this
// script never runs). Skipped entirely under prefers-reduced-motion,
// or if IntersectionObserver isn't supported, so headings just render
